...
::: warning
参数名不要使用 `when`、`next` 这类[通用参数](global.md)的名字，也不要使用 `writeReturnTo`。这些名字会先被引擎自己消费掉，不会得到你期望的效果。编辑器会对这种写法给出警告。
:::

### writeReturnTo
- 字符串

指定被调用场景的[返回值](return.md)写回调用方的哪个变量，写入方式与不带参数的 `setVar` 相同。
